...
// MID is an arbitrary number for signing messages. This will make it
...
func ToSignatureBytes(v, r, s *big.Int) []byte {
	...
	sig[64] = byte(v.Uint64() - mID)

	return sig
}

// ToSignatureBytesWithMID converts the r, s, v values into a slice of bytes
...
func toSignatureValues(sig []byte) (v, r, s *big.Int) {
	r = big.NewInt(0).SetBytes(sig[:32])
	s = big.NewInt(0).SetBytes(sig[32:64])
	v = big.NewInt(0).SetBytes([]byte{sig[64] + mID})
